...
# --- helper for a scrollable right pane (only vertical) ---
import sys

# --- DPI scaling helpers ---
def apply_auto_scaling(root):
    """Auto-set Tk scaling based on display DPI."""
    try:
        ppi = root.winfo_fpixels("1i")
        scale = float(ppi) / 72.0
...
def set_scaling(root, value):
    """Set Tk scaling manually."""
    try:
        root.tk.call("tk", "scaling", float(value))
    except Exception:
        pass

# --- helper for a scrollable right pane (only vertical) ---
class RightScrollable(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.win_id = self.canvas.create_window((0, 0), window=self.content, anchor="nw")
...
# --- Matplotlib freeze: keep text sizes fixed regardless of Tk scaling ---
def freeze_matplotlib_scaling():
    try:
...
# --- Spyder/IPython safety: reset Tk default root if it lingered from a previous run ---
def _force_clean_tk_default_root():
    try:
...
def load_data(filepath):
    ...
    if filepath.endswith(".xlsx"):
        ...
        df = pd.read_csv(filepath, sep=None, engine="python", skiprows=0, header=None)  # Load .txt file, skip header
...
def load_files_and_compute_conductance(rescale=False):
    ...
    try:
        ...
        def view_selected_iv_curve():
                ...
                # Annotate the full conductance on top-left of main plot
                text_x_main = ax_view.get_xlim()[0] + 0.05 * (ax_view.get_xlim()[1] - ax_view.get_xlim()[0])
                text_y_main = ax_view.get_ylim()[1] - 0.05 * (ax_view.get_ylim()[1] - ax_view.get_ylim()[0])
                ax_view.text(text_x_main, text_y_main,
                ...
                # Input boxes for sub-region voltage range
                input_frame = tk.Frame(view_frame)
                input_frame.pack(pady=6)
                tk.Label(input_frame, text="V_min:").grid(row=0, column=0)
                entry_vmin = tk.Entry(input_frame, width=10)
                entry_vmin.grid(row=0, column=1, padx=5)
                tk.Label(input_frame, text="V_max:").grid(row=1, column=0)
                entry_vmax = tk.Entry(input_frame, width=10)
                entry_vmax.grid(row=1, column=1, padx=5)
...
def run_drift_diffusion_experiment(root, right_frame):
    ...
    def upload_and_plot_iv():
        ...
        try:
            ...
            ax.axhline(0, color="gray", linestyle="--")
            ax.axvline(0, color="gray", linestyle="--")
            ax.plot(v_mid, 0, "o", color="gold", label="Zero-current point (Em)")
            ax.plot(etotal_left, 0, 'o', color='red', label="Left Em")
            ax.plot(etotal_mid, 0, 'o', color='gold', label="Mid Em")
            ax.plot(etotal_right, 0, 'o', color='blue', label="Right Em")
...
# ADDED: Clean any lingering Tk root (safe-guarded)
try:
    _ = _force_clean_tk_default_root  # name check
    _force_clean_tk_default_root()
except Exception:
    pass

root = tk.Tk()

# ADDED: Optionally enforce a fixed Tk scaling if you defined _FORCE_TK_SCALING above
try:
    root.tk.call("tk", "scaling", _FORCE_TK_SCALING)  # e.g., 1.0
except Exception:
    pass

# Baseline scaling for later rowheight adjustment
try:
    initial_scaling = float(root.tk.call("tk", "scaling"))
except Exception:
    initial_scaling = 1.0

# ADDED: Freeze Matplotlib so plot size and plot text stay constant this run
def _freeze_matplotlib_scaling():
    try:
        import matplotlib as mpl
        try:
            import matplotlib.pyplot as _plt
            _plt.rcParams.update(_plt.rcParamsDefault)  # wipe residue from prior runs
        except Exception:
            pass
        mpl.rcParams.update({
            "figure.dpi": 100,
...
# ADDED: apply the Matplotlib freeze now
_freeze_matplotlib_scaling()

# Capture baseline named font sizes (optional; not used here but harmless)
_named_fonts = {}
try:
    import tkinter.font as tkfont  # guard import
except Exception:
    tkfont = None
...
for fname in ("TkDefaultFont", "TkTextFont", "TkFixedFont",
    ...
    try:
        if tkfont is not None:
            ...
            _named_fonts[fname] = f.cget("size")
    except Exception:
        pass

# Treeview baseline row height (fallback 20 if not available)
try:
    _style = ttk.Style()
    _base_rowheight = _style.lookup("Treeview", "rowheight") or 20
    _base_rowheight = int(_base_rowheight)
except Exception:
    _base_rowheight = 20

# ADDED: Optionally reset a few ttk metrics up-front (harmless)
try:
    _style.configure("Treeview", rowheight=20)
except Exception:
    pass

root.withdraw()

if show_intro_popup(root):
    root.deiconify()
    ...
    right_container = RightScrollable(main_frame)
    right_container.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=10, pady=10)

    # Dynamic content area where your plots/tables/forms go
    # (keep using `right_frame` everywhere else in your code)
    right_frame = tk.Frame(right_container.content)
    right_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

    # --------------------------
    # Helpers: refresh + busy UI
    # --------------------------
    def refresh_right_pane():
        """Force right pane to recompute layout and scrollbar bounds."""
        try:
            right_container.content.update_idletasks()
            right_container.canvas.itemconfigure(
                right_container.win_id,
                width=right_container.canvas.winfo_width()
            )
            right_container.canvas.configure(
                scrollregion=right_container.canvas.bbox("all")
            )
        except Exception:
            pass

    def set_busy(is_busy=True):
        """Show/clear a busy cursor so the user knows something is running."""
        try:
            cur = "watch" if is_busy else ""
            root.configure(cursor=cur)
            right_container.canvas.configure(cursor=cur)
            right_frame.configure(cursor=cur)
            left_frame.configure(cursor=cur)
            root.update_idletasks()
        except Exception:
            pass

    def call_with_busy(fn, *args, **kwargs):
        """
        Run a function synchronously (Tk-safe), show busy cursor,
        then refresh the right pane when it finishes.
        """
        set_busy(True)
        try:
            fn(*args, **kwargs)
    ...
    # --------------------------
    # Manual UI scale (left column)
    # --------------------------
    ui_scale_var = tk.StringVar(value=str(initial_scaling))

    def apply_manual_scale():
        """Apply global UI scale and immediately refresh the right pane."""
        try:
            val = float(ui_scale_var.get())
        except Exception:
            messagebox.showerror("Scale Error", "Please enter a valid number, e.g. 1.00")
            return

        # Clamp to a safe range
        val = max(0.6, min(val, 3.0))
        ui_scale_var.set(f"{val:.2f}")

        set_busy(True)
        try:
            # 1) Apply Tk scaling (fonts and widgets scale)
            set_scaling(root, val)

            # 2) Keep Treeview rows readable after scale
            try:
                factor = val / initial_scaling if initial_scaling else 1.0
                style = ttk.Style()
                new_rh = max(14, int(round(_base_rowheight * factor)))
                style.configure("Treeview", rowheight=new_rh)
            except Exception:
                pass

            # 3) Refresh layout/scrollbars on the right
            refresh_right_pane()
        finally:
            set_busy(False)

    scale_row = tk.Frame(left_frame, bg="lightgray")
    scale_row.pack(fill=tk.X, pady=(0, 8))
    tk.Label(scale_row, text="UI Scale", bg="lightgray").pack(side=tk.LEFT, padx=(0, 6))
    tk.Entry(scale_row, textvariable=ui_scale_var, width=10).pack(side=tk.LEFT)
    tk.Button(scale_row, text="Apply", command=apply_manual_scale).pack(side=tk.LEFT, padx=(6, 0))

    # === Left pane buttons (wrapped with busy helper) ===
    btn_nano_details = tk.Button(
        left_frame, text="Enter Nanochannel Details",
        command=lambda: call_with_busy(show_nanochannel_inputs), width=40
    )
    btn_nano_details.pack(pady=4)

    btn_electrolyte = tk.Button(
        left_frame, text="Enter Electrolyte Details and Analysis",
        command=lambda: call_with_busy(enter_electrolyte_details, right_frame), width=40
    )
    btn_electrolyte.pack(pady=4)

    btn_load = tk.Button(
        left_frame, text="Run I-V Analysis",
        command=lambda: call_with_busy(load_files_and_compute_conductance), width=40
    )
    btn_load.pack(pady=4)

    btn_analysis = tk.Button(
        left_frame, text="Analysis about Concentration",
        command=lambda: call_with_busy(open_analysis_ui), width=40
    )
    btn_analysis.pack(pady=4)

    btn_drift = tk.Button(
        left_frame, text="Run Drift-Diffusion Experiment",
        command=lambda: call_with_busy(run_drift_diffusion_experiment, root, right_frame), width=40
    )
    btn_drift.pack(pady=4)

    btn_reference = tk.Button(
        left_frame, text="Show Reference List",
        command=lambda: call_with_busy(show_reference_window), width=40
    )
    ...
    # ADDED: initial layout pass so the scrollbar is correct before first action
    try:
        refresh_right_pane()
    except Exception:
        pass
